<?php
namespace Hepsiantep\Ai\Publication;
...
final class CanonicalPublicationPolicy
{
    public function prepare(CanonicalProductDTO $dto): array
    {
        ...
        if (($data['context']['pricing_context']??null)!=='public_catalog_tax_included') {
            throw new \DomainException('Canonical product is not built with a public catalog pricing context.');
        }
    ...
    private function assertEvidenceForSpecs($specs,array $evidence,string $class):void
    {
        ...
        foreach (array_keys($specs) as $propertyKey) {
            $records=is_array($evidence[$propertyKey]??null)?$evidence[$propertyKey]:[];
            $valid=false;
            foreach ($records as $record) {
                if (is_array($record)&&($record['evidence_class']??null)===$class&&($record['status']??null)==='active'&&($record['is_public']??false)===true) { $valid=true; break; }
            }
            if (!$valid) { throw new \DomainException(sprintf('Property %s has no active public %s evidence.',$propertyKey,$class)); }
        }
    }
    ...
    private function publicEvidence(array $evidence):array
    {
        ...
        foreach ($evidence as $key=>$records) {
            ...
            foreach ($records as $record) {
                if (!is_array($record)||($record['status']??null)!=='active'||($record['is_public']??false)!==true) { continue; }
                unset($record['notes']);
                $result[$key][]=$record;
            }
        }
