...
final class CanonicalBuilder
{
    ...
    public function build(int $idProduct, int $idProductAttribute = 0, ?\Context $context = null): CanonicalProductDTO
    {
        ...
        if ($idProductAttribute > 0) {
            ...
            if (!\Validate::isLoadedObject($combination) || (int) $combination->id_product !== $idProduct) { throw new \InvalidArgumentException('Combination does not belong to product.'); }
        }
        ...
        if ((int) $product->id_manufacturer > 0) {
            ...
            if (\Validate::isLoadedObject($manufacturerObject)) { $manufacturer = (string) $manufacturerObject->name; }
        }
        ...
        return new CanonicalProductDTO([
            ...
            'context' => ['language' => (string) ($context->language->iso_code ?? 'en'), 'currency' => $currency, 'pricing_context' => 'public_tax_included'],
            ...
            'evidence' => $evidence,
            'links' => ['canonical_web' => $context->link->getProductLink($product), 'realtime_api' => null, 'direct_cart' => null, 'image' => $this->coverImage($product, $context->link)],
        ]);
    }
