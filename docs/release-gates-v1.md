...
- atomic gzip snapshot publication (`.jsonl.gz`)
...
2. OpenAI feed runtime configuration/storage and scheduled full-snapshot job
3. OpenAI SFTP delivery adapter plus delivery/audit status tracking
...
8. enabled CI execution in the fork (workflow is configured but GitHub currently reports no runs)
